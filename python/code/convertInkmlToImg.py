...
def parse_inkml(inkml_file_abs_path):
    if inkml_file_abs_path.endswith('.inkml'):
        ...
        traces_all_list = [{'id': trace_tag.get('id'),
                            'coords': [
                                [round(float(axis_coord)) if float(axis_coord).is_integer()
                                 else round(float(axis_coord) * 10000) for axis_coord in coord[1:].split(' ')]
                                if coord.startswith(' ')
                                else [round(float(axis_coord))
                                      if float(axis_coord).is_integer()
                                      else round(float(axis_coord) * 10000) for axis_coord in coord.split(' ')
...
def draw_pattern(traces, pattern_drawn, box_axis_size):
    ...
    if len(traces) == 1:
        ...
        for pt_idx in range(len(traces) - 1):
            ...
            one_line_x, one_line_y = line(r0=traces[pt_idx][1], c0=traces[pt_idx][0],
                                          r1=traces[pt_idx + 1][1], c1=traces[pt_idx + 1][0])

            lines_x = np.concatenate(
                [one_line_x, one_line_x, one_line_x + 1])
            lines_y = np.concatenate(
                [one_line_y + 1, one_line_y, one_line_y])

            lines_x[lines_x < 0] = 0
            lines_x[lines_x >= box_axis_size] = box_axis_size - 1

            lines_y[lines_y < 0] = 0
            lines_y[lines_y >= box_axis_size] = box_axis_size - 1

            pattern_drawn[lines_x, lines_y] = 0.0
            # pattern_drawn[ one_line_x, one_line_y ] = 0.0
...
# It returns an np.array containing imgs generated from the hypotheses
#   the first dimension is the number of hypotheses
#   the seconds and third dimension are the 2D dimensions (dim x dim)
#  See the function called gen_hypotheses_example for more info.
def gen_img_from_lg_hypotheses(inkmltraces, lg, dim):
    if type(lg) == str:
        lg = lg.split("\n")
    #  Get list of stroke ids selected for each hypothetical symbol in lg
    sym_list = parse_lg(lg)
    # Get traces belonging to each hypothetical symbol in sym_list
    traces_sel_list = get_strokes_from_lg(inkmltraces, sym_list)
    imgs = np.ndarray([len(lg), dim, dim])
    for idx, trace in enumerate(traces_sel_list):
...
def parse_lg(lg):
    ...
    for lg_line in lg:
        ...
        lg_line = lg_line.replace(" ", "").split(',')
        if lg_line[0] != "O":
            continue
        #  Select symbol id and associated stroke id (only integers, we remove the letters)
        sym.append([lg_line[1][3:], [l[1:] for l in lg_line[4:]]])
    return sym


def get_strokes_from_lg(inkmltraces, sym_list):
    strokes = []
    for sym in sym_list:
...
def gen_hypotheses_example(file):
    ...
    print("gen_hypotheses_example: file =", file)
    ...
    imgs = gen_img_from_lg_hypotheses(traces, lg, 28)
...
if __name__ == '__main__':
    ...
    # gen_hypotheses_example(sys.argv[1]):exit()
